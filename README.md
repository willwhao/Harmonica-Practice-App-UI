...
> 当前版本已完成七阶段路线的第 1–4 阶段，并推进第 5 阶段“真实音频与录音”和第 6 阶段“合规与发布质量”。项目已具备 SQLite API、云端账户、短时访问令牌、HttpOnly 刷新令牌轮换、密码重置邮件、账户注销、增量历史同步、录音回放、上传音频识谱、临时练习谱、基础隐私控制和版本化法律文本；算法仍需真实设备校准，生产部署、原曲授权谱面与版权伴奏尚未接入，因此还不能视为正式发布版本。
...
- [x] 真实埋点、报表、功能开关、灰度发布和生产监控平台。（基础版：远程开关、稳定灰度和运营摘要；完整告警/后台/自动回滚待接入。）
...
- 本地已有统一质量门禁，GitHub Actions 已接入自动执行；覆盖率阈值、生产监控告警策略仍未配置。
