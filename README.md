...
- 功能开关集中在 `src/app/ops/releaseConfig.ts`，支持默认值和环境变量覆盖。
- 生产通道默认关闭“上传音频识谱”和“运营状态面板”，避免实验性或版权敏感功能误入正式发布。
- 账户页会显示当前发布通道、版本号、已开启开关数量和每个功能开关的来源。
- 当前为本地/环境变量配置；后续可扩展为远程配置、灰度比例、用户分组和 A/B 实验。
...
- 增加新手引导：练习目标、口琴类型、麦克风用途说明和设备检查。
- 同步书签、课程进度和难点复练数据到云端。
- 接入正式错误监控与告警，例如 Sentry 或同类平台。
- 补齐正式隐私政策、用户协议、版本号和生效日期。

#### P3：正式运营与多端扩展（2 月以后）

- 接入原曲版权伴奏、教师示范音轨、OSS/CDN 和版权授权管理。
- 建立内容 CMS，用于曲库、谱面、伴奏、课程和活动管理。
- 做灰度发布、远程功能开关、运营报表和生产监控。
