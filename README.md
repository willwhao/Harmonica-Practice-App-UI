...
> 当前版本已完成七阶段路线的第 1–4 阶段，并推进第 5 阶段“真实音频与录音”、第 6 阶段“合规与发布质量”和第 7 阶段“运营与正式发布”的基础能力。项目已具备 SQLite API、云端账户、短时访问令牌、HttpOnly 刷新令牌轮换、密码重置邮件、账户注销、增量历史/学习同步、口琴校准向导、新手引导、录音回放、上传音频识谱、临时练习谱、基础隐私控制、版本化法律文本、内容 CMS、远程功能开关、运营摘要、经验值/徽章/月报摘要；算法仍需真实设备标定，生产部署、完整后台权限、告警自动化和真实授权音频文件尚未接入，因此还不能视为正式发布版本。
...
- ✅ 已增加麦克风/上传音频/练习录音的数据处理说明和隐私状态清单；隐私政策与用户协议 v1.0.0 已补齐。
- ✅ 本地账户与云端账户注销已实现；JSON 导出当前基于本机同步缓存。

### 2. 首次使用与新手引导

- 🧪 选择练习目标、口琴类型、熟练程度和每日目标已实现基础版。
- 🧪 麦克风用途说明和设备检查已在新手引导中提供基础版。
- 🧪 口琴校准向导已提供环境校准和个人音高校准 profile。
- 🗓️ 完整新手教程、提醒时间和更细的目标分层仍待完善。
- 🗓️ 吹吸、孔位、动态谱面和判定机制的新手教程仍待完善。
...
- ✅ 已实现整曲/A 段/B 段、自定义 A-B 小节、1–3 次区间重复、难点小节收藏和一键专项复练。
...
- 🧪 已实现环境噪声门、输入电平反馈和个人音高校准 profile；输入增益、延迟补偿和多设备标定仍待完善。
- 🗓️ 针对十孔口琴的吹吸、压音、超吹识别。
- 🧪 已有半音阶滑键、孔位和音高映射数据；真实滑键动作识别仍待实现。
- ✅ 已根据音准、节奏、稳定性和持续输入生成综合判定。
- ✅ 已处理权限拒绝、无输入设备、浏览器不支持和一般启动错误状态；设备中断后的自动恢复仍待增强。
...
- 🧪 已建立授权音频和 CDN 门禁模型；真实授权音频文件、变速不变调和波形同步仍待接入。
...
- 🧪 已实现本地周趋势、练习时长统计和月报摘要；周报推送和可分享图片待完善。
- 🧪 已实现最弱小节识别、错音明细和一键专项复练；长期薄弱孔位统计待完善。
- 🧪 经验值、等级、徽章和月报摘要已实现基础版；连续练习挑战和活动任务仍待后台配置。
- 🗓️ 成绩卡图片生成、隐私控制和社交分享。

### 8. 个人中心与设置

- 🧪 已支持昵称、默认口琴、当前水平和每日目标；头像和个人简介仍待完善。
- 🗓️ 每日目标、练习提醒和通知管理。
- 🗓️ 音频设备、麦克风灵敏度、判定宽松度和延迟校准。
- 🧪 已支持键盘焦点、减少动画偏好和练习状态播报；字体大小、主题切换和更多无障碍设置待完善。
- 🧪 已支持云同步和本地缓存；离线内容、存储空间管理仍待完善。
- 🧪 已支持版本化隐私政策/用户协议和数据导出/注销；反馈、帮助中心和更完整隐私管理仍待完善。

### 9. 后台运营与内容管理

- 🧪 曲目、谱面、课程和授权音频 CMS API 已有基础版；完整运营后台 UI、用户管理和权限仍待完善。
- 🧪 内容发布状态、revision 冲突保护和授权音频状态模型已实现基础版；审核流和版权工作台仍待完善。
- 🗓️ 推荐位、活动、公告、消息和练习任务配置。
- 🗓️ 用户反馈、异常识别日志和设备兼容性统计。
- 🧪 运营摘要 API 已覆盖用户、近 7 天活跃、练习、学习、CMS 内容和远程开关数量；留存、转化和告警看板仍待完善。
...
| 新手引导 | 🧪 | 学习目标、口琴类型、麦克风说明、设备检查和每日目标基础版 |
| 曲库首页 | ✅ | 搜索、筛选、最近练习、选曲 |
...
| 运营后台 | 🧪 | CMS API、远程开关和运营摘要基础版；完整后台 UI 与权限待完善 |
...
主要 API：`/api/health`、`/api/auth/register`、`/api/auth/login`、`/api/auth/refresh`、`/api/auth/forgot-password`、`/api/auth/reset-password`、`/api/auth/me`、`/api/history`、`/api/history/sync`、`/api/release-config` 和 `/api/ops/summary`。
...
当前预算为首屏 JS 不超过 470KB、CSS 不超过 110KB；超过阈值时脚本会以失败状态退出，方便后续接入 CI。
...
- `VITE_RELEASE_CHANNEL` 支持 `development`、`preview`、`production` 三种发布通道。
- `VITE_APP_VERSION` 用于在账户页展示当前应用版本。
- 功能开关集中在 `src/app/ops/releaseConfig.ts`，支持默认值、远程配置、灰度比例和环境变量覆盖。
- 生产通道默认关闭“上传音频识谱”和“运营状态面板”，避免实验性或版权敏感功能误入正式发布。
- 账户页会显示当前发布通道、版本号、已开启开关数量和每个功能开关的来源。
- 云端模式可通过 `/api/release-config` 读取远程开关，并通过稳定灰度桶按用户或设备逐步开放能力。
- `/api/ops/summary` 提供基础运营摘要：用户数、近 7 天活跃、练习次数、平均成绩、学习数据、CMS 内容和远程开关数量。
- 当前已具备基础远程开关和报表底座；后续仍需管理员权限、操作审计、告警、持久化监控报表和自动回滚。
...
- 已提供数据导出、账户注销、隐私政策和用户协议；正式商业发布前仍建议按目标地区完成法律复核。
...
- 音频识别已接入 Web Worker 最小原型，当前仍由主线程采集音频帧并把高负载 pitch detection 转交 Worker；AudioWorklet 作为后续低延迟升级路线。
...
当前路线不再切换到 Vue，继续使用 React + TypeScript + Vite 推进。原因是现有 7 个主要页面、hooks、测试体系、云同步和发布门禁已经成型，切换框架会带来接近重写的成本；现阶段真正需要解决的是架构拆分、音频稳定性、真实设备校准和生产运营化。

后续开发以 [docs/ROADMAP.md](./docs/ROADMAP.md) 为详细路线图，README 保留摘要版执行计划。

### 近期开发计划

#### P0：架构整理，先稳住地基（1–2 周）

- [x] 接入 `react-router`，替换 `App.tsx` 里手工 `useState` 切页。
- [x] 建立 URL 路由：`/`、`/prep/:songId`、`/practice`、`/results`、`/learning`、`/account`。
- [x] 支持浏览器后退、刷新和基础分享链接。
- 拆分 `PracticePage.tsx`，已先拆出孔位引导、传统简谱和控制栏；接下来继续拆：
  ...
  - [x] `PracticeHoleGuide`：孔位条、吹吸提示和当前孔位。
  - [x] `PracticeControls`：播放、暂停、麦克风、节拍器和音量控制。
- [x] 清理未使用依赖，移除当前业务未引用的 UI 模板目录及 Radix/MUI/Recharts 等运行时包，降低包体和维护成本。

#### P1：核心体验增强（2–4 周）

- [x] 抽离框架无关的练习引擎到 `src/engine/`：已迁入谱面校验/迁移、分段循环、得分判定、错音分析、回放片段和谱面布局等第一批核心逻辑。
- [x] 评估将音高检测迁移到 AudioWorklet 或 Web Worker：已选 Web Worker 先行，并接入最小 Worker 检测原型；详见 `docs/AUDIO_THREADING_PLAN.md`。
- [x] 补全曲库内容：已让 8 首歌曲拥有独立练习编配，剩余歌曲保留 fallback。
- [x] 加深 E2E：新增短区间完整练习、麦克风关闭路径、本地账户/同步设置入口覆盖。
- [x] 准备生产后端迁移方案：SQLite/sql.js 原型保留，正式环境规划 PostgreSQL、备份和 HTTPS；详见 `docs/PRODUCTION_BACKEND_PLAN.md`。

#### P2：产品化能力（1–2 月）

- 实现口琴校准向导，建立个人音高校准表，解决不同口琴、麦克风和吹奏力度导致的系统偏差。（基础版已完成：本地保存 profile，并用于练习判定偏移修正。）
- 增加新手引导：练习目标、口琴类型、麦克风用途说明和设备检查。（基础版已完成：首次无历史用户自动进入引导，完成后保存偏好。）
- 同步书签、课程进度和难点复练数据到云端。（基础版已完成：新增学习同步 API，支持难点书签和课程进度快照跨端合并。）
- 接入正式错误监控与告警，例如 Sentry 或同类平台。（基础版已完成：全局错误捕获、本地缓冲、采样上报和服务端接收接口。）
- 补齐正式隐私政策、用户协议、版本号和生效日期。（v1.0.0 已完成，账户页可查看。）

#### P3：正式运营与多端扩展（2 月以后）

- 接入原曲版权伴奏、教师示范音轨、OSS/CDN 和版权授权管理。（基础版已完成：授权登记模型、CDN URL 生成与可用性门禁。）
- 建立内容 CMS，用于曲库、谱面、伴奏、课程和活动管理。（基础版已完成：统一内容表、公开读取、登录写入和静态 fallback。）
- 做灰度发布、远程功能开关、运营报表和生产监控。（基础版已完成：远程开关表/API、稳定灰度桶、运营摘要 API 和前端客户端。）
- 评估 Capacitor 打包 Android/iOS；小程序方向保留为后期方案，第一版可降级为跟谱和练后评分。
...
- [x] 报表、功能开关、灰度发布和生产监控基础版。（已具备远程开关、稳定灰度、运营摘要和监控事件接收；完整真实埋点、告警/后台/自动回滚待接入。）
- [x] 经验值、徽章和月报摘要基础版。
...
- 当前自相关算法已支持 Web Worker 执行；正式版本可继续评估 AudioWorklet 采集管线。
- 歌曲 1、2、3、7 已配置独立练习编配；其余歌曲仍会回退到 C 调基础练习谱。
- 当前谱面是用于产品与判定验证的原创练习编配，不代表对应商业歌曲的官方完整曲谱；正式内容上线前仍需核对授权和版权。
- 已有同步合成练习伴奏、独立音量、传统两行简谱和授权音频/CDN 门禁模型；真实授权音频文件、耳机监听和真实音频波形同步尚未实现。
...
- 本地已有统一质量门禁，GitHub Actions 已接入自动执行；覆盖率阈值、生产监控告警策略和后台权限审计仍未配置。
- 项目已初始化 Git 并推送至 GitHub；CI/CD 已有质量门禁和基础灰度开关能力，正式部署和自动回滚仍待接入。
