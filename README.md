...
> 当前版本已完成七阶段路线的第 3 阶段“工程质量”。项目已具备严格 TypeScript 检查、ESLint、单元/组件/E2E 测试、PCM 音频回归、Chrome/Edge 移动视口验证和统一质量门禁；算法仍需真实设备校准，后端、原曲授权谱面与版权伴奏尚未接入，因此还不能视为正式发布版本。
...
- ✅ 可选择十孔或半音阶口琴。
- ✅ 可选择动态下落或传统乐谱模式。
- 🧪 速度、分段、循环、节拍器和合成伴奏已接入；口琴类型和传统乐谱模式仍需独立实现。
...
- 🧪 邮箱和密码注册、登录已完成本地原型；正式版仍需服务端认证。
- 🗓️ 手机号注册、验证码验证和快捷登录。
- 🗓️ 微信、Apple、Google 等第三方登录，可按发布平台选择接入。
- 🧪 退出登录和本机会话管理已实现；忘记密码、重置密码和全部设备下线待后端支持。
- 🗓️ Access Token / Refresh Token 会话管理及过期刷新。
- 🗓️ 用户协议、隐私政策、麦克风用途说明和数据删除入口。
- 🧪 本地账户删除与 JSON 数据导出已实现；正式云端注销流程待接入。
...
- 🗓️ 原曲版权伴奏、示范音轨和真实音频变速。
- 🗓️ 伴奏、示范音、麦克风监听的独立音量控制。
- 🗓️ 倒计时、预备拍、变速不变调和音画同步。
- 🗓️ 耳机模式、扬声器模式和蓝牙延迟校准。
- 🗓️ 自动暂停、后台恢复和音频焦点处理。

### 7. 成绩分析与成长体系

- 🧪 已按音符和小节展示音准偏差与错音分布；节奏和稳定性维度待完善。
- 🗓️ 回放本次录音，并定位到具体错误音符。
...
- **动画**：Motion
- **当前数据存储**：浏览器 `localStorage`，账户密码使用 PBKDF2 加盐校验值
- **音频能力**：Web Audio API、自相关基础音高检测；后续计划迁移高负载分析到 AudioWorklet
- **建议后端方案**：Node.js/NestJS 或其他支持 REST/GraphQL 的服务端框架
- **建议数据库**：PostgreSQL；Redis 用于验证码、会话和热点缓存
...
该命令依次执行严格类型检查、Lint、28 项 Node 单元测试、4 项组件测试、4 项跨浏览器 E2E 和生产构建。

## 数据与隐私原则

- 当前练习记录仅保存在本机浏览器中，不会上传服务器。
- 接入登录后，应明确区分本地数据、云端数据和用户录音。
- 采集麦克风前必须展示用途并获得用户授权。
- 用户录音默认不应上传；如需云端分析，应提供单独授权和删除机制。
- 登录凭证不得写入 `localStorage`，正式版本应使用安全 Cookie 或受保护的令牌存储方案。
- 应提供隐私政策、用户协议、数据导出和账户注销能力。
...
| 4. 后端与云同步 | API、数据库和数据模型 | 正式认证、找回与令牌安全 | 云端历史、跨设备同步和冲突处理 | ⏳ 下一阶段 |
| 5. 真实音频与录音 | 授权伴奏和示范音轨 | 音画同步、变速和独立音量 | 录音、回放和错音定位 | ⬜ 待开始 |
| 6. 合规与发布质量 | 隐私、协议、导出与注销 | 无障碍、多设备与浏览器兼容 | 性能预算、监控和异常恢复 | ⬜ 待开始 |
| 7. 运营与正式发布 | 内容后台、审核和版权管理 | 埋点、报表与功能开关 | CI/CD、灰度发布和回滚 | ⬜ 待开始 |
...
- 已实现本地注册、登录和账户管理原型，但尚无后端身份服务、验证码、密码找回或跨设备会话。
...
- 已有同步合成练习伴奏，但原曲版权伴奏、独立音量、耳机监听和传统乐谱尚未实现。
- 最近练习记录只存在当前浏览器，清理站点数据后会丢失。
- 已有严格类型检查、Lint、单元/组件测试及 Chrome/Edge 关键流程 E2E；麦克风授权、真实音频设备和 Safari 仍需专门测试。
- 本地已有统一质量门禁，但 CI 托管执行、覆盖率阈值和错误监控仍未配置。
- 当前目录尚未初始化为 Git 仓库。
